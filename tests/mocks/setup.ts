...
import { TestProvider } from './accounts'
...
export async function setupDappUser (env, dapp_user, provider, dapp): Promise<string> {
    ...
    if (providerOnChain) {
        ...
        if (captchaData.merkle_tree_root !== providerOnChain.captchaDatasetId.toString()) {
            throw (`Cannot find captcha data id: ${providerOnChain.captchaDatasetId.toString()}`)
        }
        const commitment_id = tree.root!.hash
        ...
        if (typeof (dapp.contractAccount) === 'string' && typeof (provider.address) === 'string') {
            ...
            await tasks.dappUserCommit(dapp.contractAccount, providerOnChain.captchaDatasetId, commitment_id, provider.address)
            const commitment = await tasks.getCaptchaSolutionCommitment(commitment_id)
