...
describe("PROVIDER TASKS", () => {
  ...
  before(async () => {
    ...
    const [providerMnemonic, providerSigner] =
      await mockEnv.createAccountAndAddToKeyring();
  ...
  async function setup() {
    ...
    let captchaSolutions: CaptchaSolution[] = SOLVED_CAPTCHAS.map(
      (captcha) => ({
  ...
  it("Captcha proofs are returned if commitment found and solution is correct", async () => {
    ...
    let captchaSolutionsSalted = captchaSolutions.map((captcha) => ({
      ...captcha,
      salt: salt
    }));
    let captchasHashed = captchaSolutionsSalted.map((captcha) =>
    ...
    let result = await tasks.dappUserSolution(
      DAPP_USER.address,
      DAPP.contractAccount!,
      requestHash,
  ...
  it("Dapp User sending an invalid captchas causes error", async () => {
    ...
    let tree = new CaptchaMerkleTree();
    let captchasHashed = captchaSolutions.map((captcha) =>
      computeCaptchaSolutionHash(captcha)
    );
    tree.build(captchasHashed);
    let solutionPromise = tasks.dappUserSolution(
      DAPP_USER.address,
      DAPP.contractAccount!,
      requestHash,
  ...
  it("No proofs are returned if commitment found and solution is incorrect", async () => {
    ...
    let tree = new CaptchaMerkleTree();
    let captchasHashed = captchaSolutionsBad.map((captcha) =>
      computeCaptchaSolutionHash(captcha)
    );
    tree.build(captchasHashed);
    let commitmentId = tree.root!.hash;
    ...
    let result = await tasks.dappUserSolution(
      ...
      DAPP.contractAccount!,
      requestHash,
      JSON.parse(JSON.stringify(captchaSolutionsBad))
    );
